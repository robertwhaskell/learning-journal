...
def details(request):
    ...
    entry_from_db = Entry.by_id(request.matchdict.get('id', -1))
    entry = [{
        'title': entry_from_db.title,
        'text': markdown.markdown(entry_from_db.text, extensions=('codehilite', 'fenced_code')),
        'created': entry_from_db.created,
        'id': entry_from_db.id
        }]
